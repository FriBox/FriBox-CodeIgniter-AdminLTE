...
class Admin extends FWController {
    ...
    public function index() {  //系统首页；
        ...
        switch ($xChkLogin['xFWChkLogin'] ?? '-1') {
        case 0:
            //0：验证成功；
            $Data = array_merge($Data, $xChkLogin);
            break;
        default:
            //-1：验证失败；
            echo "<meta http-equiv='Refresh' content='0;URL=/Admin/Login'>";  return;
        }
    ...
    public function login() {  //登录页面；
        ...
        if ($this->request->getMethod() == 'post') {
            ...
            echo "<meta http-equiv='Refresh' content='1;URL=/Admin'>";  return;
        } else {
            // 处理get请求，输出登录页面
            $Data['xFWErr']='';  //输出信息；
            log_message('debug', 'Route::Admin/Login'); return view('Admin/Login',$Data);  //输出页面；
    ...
    public function logout() {  //登出页面；
        ...
        log_message('debug', 'Route::Admin/Logout');  return 'AAA';  //输出页面；
